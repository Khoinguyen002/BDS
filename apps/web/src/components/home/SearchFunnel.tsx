...
export const SearchFunnel = ({
  ...
  const dirtyFields = useRef<Set<"listingType" | "price" | "locations">>(
    new Set(),
  );
      ...
      parserTimer.current = setTimeout(() => {
        ...
        if (!dirtyFields.current.has("price")) {
          if (parsed.priceMin !== undefined || parsed.priceMax !== undefined) {
            const lt =
              (parsed.propertyMode?.split("-")[1] as "sale" | "rent") ||
              listingType;
            const ranges = PRICE_RANGES[lt] ?? [];
            for (const range of ranges) {
              const [rMin, rMax] = range.value.split("-").map(Number);
              const matchMin =
                parsed.priceMin === undefined || parsed.priceMin >= rMin;
              const matchMax =
        ...
        if (parsed.locationQuery && !dirtyFields.current.has("locations")) {
          ...
          const matched = locations.find((l) =>
            (l.title as string).toLowerCase().includes(locLower),
          );
          if (matched) {
  ...
  const resetAll = () => {
    ...
    router.push(
      agentSlug ? `/${locale}/${agentSlug}/apartments` : `/${locale}/search`,
    );
  };
        ...
        {compactMobile && !funnelVisible && (
          ...
          >
            ...
            >
              ...
              <CaretDownIcon
                className={`w-3.5 h-3.5 text-foreground-muted shrink-0 transition-transform duration-200 ${miniOpen ? "rotate-180" : ""}`}
              />
            </button>
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && setLocationPanelOpen(true)
                        }
                        ...
                        {selectedLocations.length === 0 ? (
                          ...
                          <div className="flex items-center gap-1 flex-1 min-w-0 overflow-hidden">
                            {selectedLocations.slice(0, 2).map((loc) => (
                              <span
                                key={loc.id}
                                className="bg-secondary/10 text-secondary text-xs font-medium px-1.5 py-0.5 shrink-0"
                              >
                                {loc.title}
                              </span>
                            ))}
      ...
      >
        <form onSubmit={handleSearch} className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-6 mb-1 px-2">
            ...
            >
              <KeyIcon
                weight={listingType === "rent" ? "fill" : "regular"}
                className="w-4 h-4"
              />
              {t("apartments.for_rent")}
            </button>
            ...
            >
              <TagIcon
                weight={listingType === "sale" ? "fill" : "regular"}
                className="w-4 h-4"
              />
              {t("apartments.for_sale")}
            </button>
          ...
          <div className="flex flex-col md:flex-row items-stretch w-full bg-background border border-border shadow-sm divide-y md:divide-y-0 md:divide-x divide-border">
            ...
            {tags.length > 0 && (
              <div className="relative md:w-56 w-full py-1 shrink-0">
                ...
                <select
                  value={tag}
                  onChange={(e) => setTag(e.target.value)}
                  className="w-full h-10 md:h-14 pl-12 pr-10 bg-transparent border-none focus:outline-none focus:ring-0 text-foreground text-sm appearance-none cursor-pointer"
                >
            ...
            <div className="relative flex-1 w-full py-1">
              ...
              <input
                ...
                onChange={(e) => handleSearchChange(e.target.value)}
                placeholder={currentPlaceholder}
                className="w-full h-10 md:h-14 pl-12 pr-5 bg-transparent border-none focus:outline-none focus:ring-0 text-foreground text-sm placeholder:text-foreground-muted"
              />
            </div>
          ...
          <div className="flex flex-col md:flex-row items-stretch w-full bg-background border border-border shadow-sm divide-y md:divide-y-0 md:divide-x divide-border">
            ...
            <div className="relative flex-1 w-full py-1">
              ...
              <div
                ...
                className="flex items-center w-full h-10 md:h-14 pl-12 pr-4 text-left hover:bg-secondary/5 transition-colors min-w-0 cursor-pointer"
              >
                {selectedLocations.length === 0 ? (
                  <span className="text-foreground-muted text-sm">
                    {t("apartments.location_trigger")}
                  ...
                  <div className="flex items-center gap-1.5 flex-1 min-w-0 overflow-hidden">
                    ...
                    {selectedLocations.length > 2 && (
                      <span className="text-xs text-foreground-muted shrink-0">
                        {t("apartments.more_locations", {
                          count: selectedLocations.length - 2,
                        })}
                      </span>
                    )}
                  </div>
