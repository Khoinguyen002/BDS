...
export async function getLandingPageByOwner(userId: string | number): Promise<LandingPage | null> {
  try {
    const data = await fetchAPI(`/landing-pages?where[owner][equals]=${userId}&depth=1`);
    if (data.docs && data.docs.length > 0) {
      return data.docs[0];
    }
    return null;
...
export async function getApartmentBySlugOrId(slugOrId: string): Promise<Apartment | null> {
  try {
    // Attempt fetch by slug first
    const data = await fetchAPI(`/apartments?where[slug][equals]=${slugOrId}&depth=2`);
    if (data.docs && data.docs.length > 0) {
      return data.docs[0];
    }
...
export async function getApartmentsByOwner(ownerId: string | number, limit: number = 6): Promise<Apartment[]> {
  try {
    const data = await fetchAPI(`/apartments?where[owner][equals]=${ownerId}&depth=1&limit=${limit}`);
    return data.docs || [];
  } catch (error) {
