...
<style>
  path, circle, rect, polygon, line, polyline { fill: ${primaryColor}; }
</style>
...
export const injectSvgColor = (svgText: string, primaryColor: string) => {
  let modifiedSvg = svgText.replace(/fill="[^"]+"/g, "");
  const styleBlock = `
<style>
  path, circle, rect, polygon, line, polyline { fill: ${primaryColor}; }
</style>
`;
  return modifiedSvg.replace(/(<svg[^>]*>)/i, `$1${styleBlock}`);
};

export async function generateFaviconResponse(settings: AppSetting | null) {
  if (
    settings?.shortLogo &&
    ...
    try {
      ...
      if (res.ok) {
        ...
        const primaryColor =
          typeof settings?.themePrimary === "string"
            ...
            : "#059669";
        svgText = injectSvgColor(svgText, primaryColor);
      ...
      console.error("Error fetching short logo", e);
    }
  }
  ...
  const primaryColor =
    typeof settings?.themePrimary === "string"
      ? settings.themePrimary
      : "#059669";
  return new NextResponse(getFallbackSvg(primaryColor), {
    headers: {
      "Content-Type": "image/svg+xml",
      "Cache-Control": "public, max-age=3600",
    },
